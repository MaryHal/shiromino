...
void scoredb_init(struct scoredb *s, const char *filename)
{
    ...
    debug("Opened scoredb %s", filename);
...
void scoredb_add(struct scoredb *s, struct replay *r)
{
    ...
    check(sqlite3_bind_int(sql,  sqlite3_bind_parameter_index(sql, ":mode"),       r->mode) == SQLITE_OK, "Could not bind parameter value: %s", sqlite3_errmsg(s->db));
    check(sqlite3_bind_int(sql,  sqlite3_bind_parameter_index(sql, ":grade"),      r->grade) == SQLITE_OK, "Could not bind parameter value: %s", sqlite3_errmsg(s->db));
    check(sqlite3_bind_int(sql,  sqlite3_bind_parameter_index(sql, ":startLevel"), r->starting_level) == SQLITE_OK, "Could not bind parameter value: %s", sqlite3_errmsg(s->db));
    check(sqlite3_bind_int(sql,  sqlite3_bind_parameter_index(sql, ":level"),      r->ending_level) == SQLITE_OK, "Could not bind parameter value: %s", sqlite3_errmsg(s->db));
    check(sqlite3_bind_int(sql,  sqlite3_bind_parameter_index(sql, ":time"),       r->time) == SQLITE_OK, "Could not bind parameter value: %s", sqlite3_errmsg(s->db));
    check(sqlite3_bind_blob(sql, sqlite3_bind_parameter_index(sql, ":replay"),     replayData, replayLen, NULL) == SQLITE_OK, "Could not bind parameter value: %s", sqlite3_errmsg(s->db));
    
    int ret = sqlite3_step(sql);
    check(ret == SQLITE_DONE, "Could not insert value into scores table: %s", sqlite3_errmsg(s->db));

    debug("Wrote replay: %s", replayDescriptor);
...
void scoredb_get_full_replay(struct scoredb *s, struct replay *out_replay, int replay_id)
{
    ...
    int ret = sqlite3_bind_int(sql, sqlite3_bind_parameter_index(sql, ":scoreId"), replay_id);
    check(ret == SQLITE_OK, "Could not bind parameter: %s", sqlite3_errmsg(s->db));
